...
class CustomDatePicker: UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        switch component {
        case 0:
            return (self.years.object(at: row) as? String)?.appending("year")
        case 1:
            return (self.months.object(at: row) as? String)?.appending("month")
        case 2:
            let index = pickerView.selectedRow(inComponent: 1)
            updateDays(month: self.months.object(at: index) as! String)
            return (self.days.object(at: row) as? String)?.appending("day")
        default:
            return "2022"
        }
