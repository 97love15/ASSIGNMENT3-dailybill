...
//        oneView.layer.contents = UIImage(named: "背景")?.cgImage
        ...
        seleteButton.setTitle("set date", for: .normal)
        ...
        spendingTitleLabel.text = "💰month payment："
        ...
        incomeTitleLabel.text = "👏month income："
    ...
    func loadData(date: String, spending: String?, income: String?) -> Void {
        ...
        if date.count == 6 {
            ...
            let year: String = String(date[date.startIndex ..< index] + "year")
            let month: String = String(date[index ..< date.endIndex] + "month: ")
            self.yearLabel?.text = year
            self.dateLabel?.title = month
        }
