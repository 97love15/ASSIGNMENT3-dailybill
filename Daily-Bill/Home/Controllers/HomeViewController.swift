...
//        billingDetailBottomView.layer.contents = UIImage(named: "1")?.cgImage
        ...
        billingDetailBottomView.dateLabel?.text = String(format: "%@ %@", CalendarHelper.dateString(date: _tallyModel?.date ?? "00000000", originFromat: "yyyyMMdd", resultFromat: "yyyyyearMMmonthddday"), CalendarHelper.weekDay(dateString: _tallyModel?.date ?? "20190103" , format:"yyyyMMdd"))
        ...
        billingDetailBottomView.delBtn {
            let actionAlert = UIAlertController(title: "Alert❗️❗️", message: "if you delete that you can not find anymore？", preferredStyle: .alert)
            actionAlert.addAction(UIAlertAction(title: "let me think", style: .destructive, handler: nil))
            actionAlert.addAction(UIAlertAction(title: "confirm", style: .default, handler: {_ in
    ...
    func loadData() -> Void {
        ...
        self.billingDetailBottomView.dateLabel?.text = String(format: "%@ %@", CalendarHelper.dateString(date: _tallyModel?.date ?? "00000000", originFromat: "yyyyMMdd", resultFromat: "yyyyyearMMmonthddday"), CalendarHelper.weekDay(dateString: _tallyModel?.date ?? "20190103" , format:"yyyyMMdd"))
