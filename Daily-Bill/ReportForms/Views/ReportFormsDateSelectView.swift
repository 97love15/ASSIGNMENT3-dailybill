...
class ReportFormsDateSelectView: UIView, UITableViewDelegate, UITableViewDataSource {
   ...
   static let years: Int = 120
    ...
    var year: NSInteger {
        set{
            _year = newValue
            self.yearLabel?.text = String(format: "%d year", _year)
            if self.handler != nil {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 || indexPath.row == 1{
                ...
                let monthString = CalendarHelper.nowDateString(dateFormat: "MM")
                let month: NSInteger = NSInteger(monthString) ?? 0
                cell.label?.text = (self.dataArray?[indexPath.row + 12 - month - 2] as! String).appending("month")
                cell.isUserInteractionEnabled = true
            }

        }else{
            cell.label?.text = (self.dataArray?[indexPath.row] as! String).appending("month")
            cell.isUserInteractionEnabled = true
        }
